...
class AuthService {
  ...
  Future<void> agreeTos(RequestsService requests) async {
    final token = await requests.put('/contact', {
      'agreeTos': true,
    }, (v) => v['token'] as String);
  ...
  Future<Contact> createContact(DateTime dob, RequestsService requests) async {
    ...
    final token = await requests.post('/contact', { 'dob': dob.toUtc().toIso8601String() }, (v) => v['token'] as String);
    await _setAuthToken(token!, requests);
    return _contact!;
  }
  ...
  Future<Profile> createProfile(
    ...
    { required bool isTransgender }
  ) async {
    if (!_isLoggedIn) throw StateError('not logged in');
    if (_contact == null) throw StateError('no contact');

    if (name.isEmpty || name.length > 128) throw ArgumentError('invalid name');
    if (bio != null && (bio.isEmpty || bio.length > 1024)) throw ArgumentError('invalid bio');
    if (gender.isEmpty || gender.length > 16) throw ArgumentError('invalid gender');
    if (relationshipInterests.isEmpty) throw ArgumentError('must have at least one relationship interest');
    ...
    _profile = await requests.post('/profile', {
      'name': name,
      'bio': bio ?? '',
      'gender': gender,
  ...
  Future<Profile> updateProfile(
    ...
    _profile = (await requests.put('/profile', {
      'name': name,
      'bio': bio,
      'gender': gender,
      'relationshipInterests': relationshipInterests.map((e) => e.toLowerCase()).toList(),
      'neurodiversities': neurodiversities.toList(),
      'interests': interests.toList(),
      'pronouns': (pronouns == null || pronouns.trim().isEmpty) ? null : pronouns,
    }, Profile.fromJson))!;
  ...
  Future<bool> isAppAvailableInArea(LocationService location, RequestsService requests) async {
    final loc = await location.getLocation();
    final available = (await requests.get('/availability', (v) => v['available'] as bool, urlParams: {
  ...
  Future<void> requestLoginCode(PhoneNumber phone, LoginCodeChannel channel, RequestsService requests) async {
    await requests.get('/login', (_) => null, urlParams: { 'phone': phone.toString(), 'channel': channel.name });
  }

  Future<void> login(PhoneNumber phone, String code, RequestsService requests) async {
    final token = (await requests.post('/login', { 'code': code, 'phone': phone.toString() }, (v) => v['token'] as String))!;
  ...
  Future<void> _setAuthToken(String token, RequestsService requests) async {
    ...
    if (jwt['sub'] != null && jwt['phn'] != null && jwt['dob'] != null && jwt['flg'] != null) {
      final dob = DateTime.fromMicrosecondsSinceEpoch(jwt['dob']);
      final flags = (jwt['flg'] as String).split('').map((e) => e == '1').toList();
  ...
  Future<void> _tryGetUserData(String authToken, RequestsService requests) async {
    _isLoggedIn = true;
    _contact = null;
    _profile = null;
    ...
    try {
      final token = (await requests.get('/login/update', (v) => v['token'] as String))!;
      requests.setAuthToken(token);
      jwt = Jwt.decode(token)?.payload;
    } on RequestsServiceHttpException catch (e) {
      if (e.status != 404) rethrow; // 404 means we have no contact. This is actually fine.
    }
